...
class Help(commands.Cog):
    ...
    async def help(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="", value="``/checkforge``: Checks the forge for the bests flips (not 100% accurate), some times it will take a few attempts for the command to work, discord api error, not mine (obviously)\n``/help``: Shows this message\n``/credits``: Just some credits of some of the thigs I used")
